...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname ex-276-itunes-refactored) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (select-album-date a d lt)
  (local (; Track -> Boolean
          (define (after-date? trk)
            (and
            (before? d (track-played trk))
            (string=? a (track-album trk)))))
  (filter after-date? lt)))
