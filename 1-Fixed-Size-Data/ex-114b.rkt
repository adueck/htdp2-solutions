...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex-114b) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; Any -> Boolean
; Checks to see if an editor is a valid editor
(check-expect (editor-valid? "a") #false)
(check-expect (editor-valid? 45) #false)
(check-expect (editor-valid? (make-editor "a" 4)) #false)
(check-expect (editor-valid? (make-editor "ab" "cd")) #true)
(define (editor-valid? a)
  (cond
    [(editor? a) (and
                  (string? (editor-pre a))
                  (string? (editor-post a)))]
    [else #false]))
...
(check-expect (edit (make-editor "" "world") "\r")
              (make-editor "" "world"))
(check-expect (edit
               (make-editor "way too much text this will not fit at all" "no") "a")
               (make-editor "way too much text this will not fit at all" "no"))
(define (edit ed ke)
  (cond
    ...
    [(string=? "\r" ke) ed]
    [(= (string-length ke) 1)
     (if (will-fit? ed ke) (add-to-pre ed ke) ed)]
...
; checks if a given charactar will fit in the GUI if added
(check-expect (will-fit?
               (make-editor "not much" " here") "a")
              #true)
(check-expect (will-fit?
               (make-editor "way too much text this will not fit at all" "no") "a")
              #false)
(define (will-fit? ed c)
  (<
   (image-width (render-text
    (string-append (editor-pre ed) c (editor-post ed))))
   WIDTH))
