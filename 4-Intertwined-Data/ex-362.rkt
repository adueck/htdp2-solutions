...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname ex-362) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
; S-expr SL -> BSL-Value
(check-expect (interpreter '(+ 3 (inc5 10))
                           '((fun inc5 x (+ x 5))))
              18)
(check-expect (interpreter '(*
                             (double (trip 10))
                             3)
                           '((con ten 10)
                             (fun double x (* x 2))
                             (fun trip x (+ x (double x)))))
              180)
(define (interpreter sexp sl)
  (eval-all (parse sexp) (parse-dsl sl)))

; SL -> BSL-da-all
(check-expect (parse-dsl '(3
                           (* 2 1)
                           (fun f x (+ 3 x))
                           (fun add5 x (+ 5 x))
                           (con a 5)
                           ...
                           (con g 23)))
              da1)
(check-error (parse-dsl '("hello")))
(define (parse-dsl sl)
  (map (lambda (s)
         (cond
           [(atom? s) (parse-atom s)]
           [(not (symbol? (first s))) WRONG]
           [(symbol=? (first s) 'fun) (make-BSL-fun-def (second s) (third s) (parse (fourth s)))]
           [(symbol=? (first s) 'con) (make-BSL-con-def (second s) (parse (third s)))]
           [else (parse s)]))
       sl))

; S-expr -> BSL-expr
; Parses an S-expr used for an expression
...
(check-expect (parse '(+ (* 2 (+ 1 2)) (+ 2 4)))
              (make-add (make-mul 2
                                  (make-add 1 2))
                        (make-add 2 4)))
(check-expect (parse '(a 3)) (make-fun 'a 3))
(check-expect (parse '(+ 3 (b 10)))
              (make-add 3 (make-fun 'b 10)))
(check-expect (parse '(doub (trip 4)))
              (make-fun 'doub
                        (make-fun 'trip 4)))
