...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname ex-327) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(check-expect (create-bst-from-list (list (list 1 'a) (list 2 'b) (list 3 'c)))
              (make-node
               3
               'c
               (make-node 2 'b (make-node 1 'a NONE NONE) NONE)
               NONE))
(check-expect (create-bst-from-list.v2 '()) NONE)
(check-expect (create-bst-from-list.v2 (list (list 1 'a) (list 2 'b) (list 3 'c)))
              (make-node
               1
               'a
               NONE
               (make-node 2 'b NONE (make-node 3 'c NONE NONE))))
(define (create-bst-from-list lons)
  (cond
    [(empty? lons) NONE]
    [else (local 
            ((define n (first (first lons)))
             (define s (second (first lons))))
            (create-bst (create-bst-from-list (rest lons)) n s))]))
(define (create-bst-from-list.v2 lons)
  (foldl
   (lambda (x tree) (create-bst tree (first x) (second x)))
   NONE
   lons))

; This second version using abstraction produces an inverted tree because it starts building the tree
; with the first item on the list, whereas the natural recursion example starts building the tree
; with the last item on the list at the end of all the recursive calls
