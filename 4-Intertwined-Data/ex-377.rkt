...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname ex-377) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require 2htdp/image)
...
(define (make-bye xe)
  (local
    ...
    (cons 'ul (map replace-in-item items))))

; XItem.v2 -> XItem.v2
(define (replace-in-item xi)
  (local
    ((define content (xexpr-content xi))
     (define child (first content)))
    (cond
      [(word? child) (if (string=? (word-text child) "hello")
                                   '(li (word ((text "bye"))))
                                   xi)]
      [else (cons 'li (cons (make-bye child) '()))])))
