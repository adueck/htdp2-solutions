...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex-198-words-by-first-letter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (words-by-first-letter d)
  (cond
    [(empty? d) '()]
    [else
     (add-word-to-sub-dicts
      (first d)
      (words-by-first-letter (rest d)))]))
...
; Shows the letter which begins the most words
; in a given dictionary along with its count
(check-expect (most-frequent.v2 TESTING-DICT)
              (most-frequent TESTING-DICT))
(define (most-frequent.v2 dl)
  (sub-dict-info
   (longest-sub-dict
    (words-by-first-letter dl))))

; String -> List-of-Sub-Dictionaries
...
(define (sort-dicts< lod)
  (cond
    [(empty? lod) '()]
    [else
     (insert-dict (first lod) (sort-dicts< (rest lod)))]))

; Sub-Dictionary -> List-of-Sub-Dictionarys
; inserts a letter-based dictionary into it's proper place in and alphabetically sorted list of Dictionaries
(check-expect (insert-dict (list "a") '()) (list (list "a")))
(check-expect (insert-dict
               (list "dart" "door")
               (list
                (list "apple")
                (list "tornado")))
  ...
  (cond
    ...
    [else
     (if
      (string<=?
       (letter-of-dict d)
       (letter-of-dict (first lod)))
      (cons d lod)
      (cons
       (first lod)
       (insert-dict d (rest lod))))]))
...
; Dicitonary -> Number
; caluclates the length of a Dictionary
(check-expect (dict-length '()) 0)
(check-expect (dict-length (list "a" "b" "c")) 3)
(define (dict-length d)
  (cond
    [(empty? d) 0]
    [else (add1 (dict-length (rest d)))]))
