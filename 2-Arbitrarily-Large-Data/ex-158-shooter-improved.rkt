...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex-158-shooter-improved) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; removes a shot once it gets off the screen
(check-expect (tock '()) '())
(check-expect (tock (cons 5 '())) (cons 4 '()))
(check-expect (tock (cons 10 (cons 5 '())))
              (cons 9 (cons 4 '())))
(check-expect (tock (cons 10 (cons 1 '())))
              (cons 9 '()))
(define (tock w)
  (cond
    [(empty? w) '()]
