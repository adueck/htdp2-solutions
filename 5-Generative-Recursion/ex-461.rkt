...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname ex-461) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(define EPSILON 0.01)
(define R 100000)
...
; Computes the area undef the graph of f between a and b an adaptive strategy
; [Number -> Number] Number Number -> Number
(check-within (integrate-adaptive constant 12 22) 200 EPSILON)
(check-within (integrate-adaptive linear 0 10) 100 EPSILON)
(check-within (integrate-adaptive square 0 10)
              (- (expt 10 3) (expt 0 3))
              EPSILON)
(define (integrate-adaptive f a b)
  (local ((define mid (/ (+ a b) 2))
          ; note: to get the tests to pass within EPSILON I had to lower the area-tolerance a wee bit
          (define area-tolerance (* 0.9 (abs (* EPSILON (- b a)))))
          (define kepler-whole (integrate-kepler f a b))
          (define kepler-halves (+
                                 (integrate-kepler f a mid)
                                 (integrate-kepler f mid b))))
    (if (< (abs (- kepler-whole kepler-halves))
           area-tolerance)
        kepler-halves
