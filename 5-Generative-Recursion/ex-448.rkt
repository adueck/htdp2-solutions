...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname ex-448) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
; Number -> Number
; roots 2 and 4
(define (poly1 x)
  (* (- x 2) (- x 4)))
...
; The distance between left and right becomes smaller (is halved) at each step
; eventually the distance is <= ε in which case we say the interval is so small that
; it is "close enough" to an exact point and we are done, we just treat the left side
; of the tiny interval as the exact point.
